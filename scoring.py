...
def compute_selection_score(metrics: dict | None) -> float:
    ...
    """
    # Захист від None / списків / рядків замість dict.
    ...
    try:
        ...
        f1 = metrics.get("f1")
        # Усі три метрики обов'язкові для формули.
...
def get_selection_score(metrics: dict | None) -> float:
    ...
    Використовується в UI / predict, де score уже може бути в JSON.
    ...
    if stored is not None:
        try:
            ...
            # Битий збережений score — падаємо назад на перерахунок.
            return 0.0
