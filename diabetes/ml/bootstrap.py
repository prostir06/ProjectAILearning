"""
Опційне навчання моделей, якщо joblib відсутній.

UI (GET /) **не** викликає цей модуль — показує підказку ``python train.py``.
``ensure_models_ready()`` лишається для скриптів / ручного cold-start.
Для продакшену комітьте готовий joblib.
"""
